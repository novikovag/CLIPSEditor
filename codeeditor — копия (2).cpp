...
void CodeEditor::mouseMoveEvent(QMouseEvent *e)
{
    if (QApplication::keyboardModifiers() == Qt::AltModifier) { // ControlModifier e->button() != Qt::LeftButton &&
        ...
        if (!columnSelection) {
            ...
            columnStartCursor = textCursor();
            columnStartRect   = cursorRect();

            columnStartCursor.clearSelection();
            //setTextCursor(columnStartCursor);
            ...
            while (1) {
                ...
                if (block.length() < columnStartCursor.positionInBlock()) {
                    selection.cursor.setPosition(block.position() + block.length() - 1);

                    if (columnSelections.isEmpty())
                        goto L;

                } else {
                    selection.cursor.setPosition(block.position() + columnStartCursor.positionInBlock());
                }
                ...
                while (1) {
                    ...
                    if (columnStartRect.center().x() < e->x()) {

                        if (selection.cursor.positionInBlock() == selection.cursor.block().length() - 1 || cursorRect().center().x() > e->x())
                           break;

                        selection.cursor.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor, 1);

                        if (selection.cursor.positionInBlock() - columnStartCursor.positionInBlock() > columnMax)
                            columnMax = selection.cursor.positionInBlock() - columnStartCursor.positionInBlock();

                    } else if (columnStartRect.center().x() > e->x()) {

                        if (selection.cursor.positionInBlock() == 0 || cursorRect().center().x() < e->x())
                            break;

                        selection.cursor.movePosition(QTextCursor::Left, QTextCursor::KeepAnchor, 1);
                    } else { // columnStartRect.center().x() == e->x(), рисуем линиию
                        goto L;
                    }
                }

                if (columnStartRect.center().y() < e->pos().y())
                    columnSelections.append(selection);
                else
                    columnSelections.prepend(selection);
            L:
                if (columnStartRect.center().y() < e->pos().y()) {
                    block = block.next();

                    if (!block.isValid() || blockBoundingGeometry(block).top() > e->pos().y())
                        break;

                } else {
                    block = block.previous();

                    if (!block.isValid() || blockBoundingGeometry(block).bottom() < e->pos().y())
                        break;
                }
            }
...
{
    if (columnSelection && e->button() != Qt::RightButton)
        columnSelectionOff();
...
void CodeEditor::contextMenuEvent(QContextMenuEvent *e)
{
    ...
    MCUT->setEnabled((columnSelection && !columnSelections.isEmpty())  || textCursor().hasSelection());
    MCOPY->setEnabled((columnSelection && !columnSelections.isEmpty()) || textCursor().hasSelection());
